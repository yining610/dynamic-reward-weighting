...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    if "accuracy" in batch.non_tensor_batch:
        reward_extra_info["multi_objective/rollout_accuracy_mean"] = batch.non_tensor_batch["accuracy"].mean().item()
    if "concise" in batch.non_tensor_batch:
        reward_extra_info["multi_objective/rollout_concise_mean"] = batch.non_tensor_batch["concise"].mean().item()
    if "format" in batch.non_tensor_batch:
        reward_extra_info["multi_objective/rollout_format_mean"] = batch.non_tensor_batch["format"].mean().item()
    ...
    metrics = {
        ...
        **(
        ...
        # response length
        "rollout_response_length/mean": torch.mean(response_length).detach().item(),
        "rollout_response_length/max": torch.max(response_length).detach().item(),
        "rollout_response_length/min": torch.min(response_length).detach().item(),
        "rollout_response_length/clip_ratio": torch.mean(torch.eq(response_length, max_response_length).float()).detach().item(),
        # prompt length
        "rollout_prompt_length/mean": torch.mean(prompt_length).detach().item(),
        "rollout_prompt_length/max": torch.max(prompt_length).detach().item(),
        "rollout_prompt_length/min": torch.min(prompt_length).detach().item(),
        "rollout_prompt_length/clip_ratio": torch.mean(torch.eq(prompt_length, max_prompt_length).float()).detach().item(),
        **reward_extra_info,
    }
    return metrics
